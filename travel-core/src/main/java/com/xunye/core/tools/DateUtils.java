...
public class DateUtils {
    ...
    public static String getWeekOfYear(Date date) {
        ...
        nowTime.setMinimalDaysInFirstWeek(4);//第一个满4天周才算一年的第一周
        simpleDateFormat.setCalendar(nowTime);

        return simpleDateFormat.format(date);
    }

    public static Map<String, List<Date>> getWeekDaysMap(Date date) {
        ...
        Map<String, List<Date>> dataCollect = new HashMap<>();
        for (int i=firstDay; i<=lastDay; i++) {
            calendarDay.set(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), i);

            String weekOfYear = getWeekOfYear(calendarDay.getTime());
            if(dataCollect.get(weekOfYear) == null) {
                dataCollect.put(weekOfYear, new ArrayList<Date>());
            }
            dataCollect.get(weekOfYear).add(calendarDay.getTime());
        }
